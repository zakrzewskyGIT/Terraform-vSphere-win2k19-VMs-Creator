...
provider "vsphere" {
  ...
  vsphere_server = var.vsphere_srv
...
resource "vsphere_virtual_machine" "vm" {
  ...
  disk {
    label            = "${var.vm-name}-${count.index + 1}_0.vmdk" #"machine.vmdk"
  ...
  disk {
    label            = "${var.vm-name}-${count.index + 1}_1.vmdk" #"machine_1.vmdk"
    unit_number      = 1
    thin_provisioned = true
    eagerly_scrub    = false
    size             = data.vsphere_virtual_machine.template.disks.0.size
